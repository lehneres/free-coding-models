...
export function createOverlayRenderers(state, deps) {
  ...
  function renderHelp() {
    ...
    const normalSec = Math.round((state.pingModeIntervals?.normal ?? 10000) / 1000)
    lines.push(`  ${key('W')}  Toggle ping mode  ${hint(`(speed 2s → normal ${normalSec}s → slow 30s → forced 4s)`)}`)
